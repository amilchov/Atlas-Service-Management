...
export default {
  ...
  methods: {
    createIncident() {
      this.$modal.show("incident-create");
    },
  ...
  mounted() {
    this.$store.dispatch("incidentTable/getAllIncidents").then(() => {
      this.dataIncidents = this.$store.state.incidentTable.incidentData;
      ...
      for (var i = 0; i < this.dataIncidents.length; i++) {
        this.data_incident_table.push({
          ...
          token_executor: this.dataIncidents[i].executor[0].token
        });
        // console.log(this.roles);
      }
      console.log("HI " + this.data_incident_table[0].number);
    });
